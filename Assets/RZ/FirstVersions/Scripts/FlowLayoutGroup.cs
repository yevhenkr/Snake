...
namespace RZ
{
    ...
    public class FlowLayoutGroup : LayoutGroup
    {
        ...
        public sizeControl childWidth = sizeControl.Manual;
        public sizeControl childHeight = sizeControl.Manual;
        ...
        public bool isChildForceExpandWidth { get { return childWidth == sizeControl.Expand; } }
        public bool isChildForceExpandHeight { get { return childHeight == sizeControl.Expand; } }
        ...
        public float SetLayout(int axis, bool layoutInput)
        {
            ...
            for (var i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (startAxis == Axis.Horizontal)
                {
                    ...
                    childSize = my_GetPreferredSize(child, 0);
                    childSize = Mathf.Min(childSize, workingSize);
                    childOtherSize = my_GetPreferredSize(child, 1);
                ...
                else if (startAxis == Axis.Vertical)
                {
                    ...
                    child = rectChildren[index];
                    childSize = my_GetPreferredSize(child, 1);
                    childSize = Mathf.Min(childSize, workingSize);
                    childOtherSize = my_GetPreferredSize(child, 0);
                    childOtherSize = Mathf.Min(childOtherSize, workingSize);
                }
                ...
                // Don't do this for the last one without expand width
                if ((i < rectChildren.Count - 1) || isChildForceExpandWidth)
                {
                    currentBarSize += spacingBetweenElements;
                }
            ...
            if (!layoutInput)
            {
                if (startAxis == Axis.Horizontal)
                {
                    ...
                    currentBarSize -= spacingBetweenElements;
                    LayoutRow(_itemList, currentBarSize, currentBarSpace, workingSize - (isChildForceExpandWidth ? 0 : spacingBetweenElements), padding.left, newOffset, axis);
                }
                else if (startAxis == Axis.Vertical)
                {
                    ...
                    currentBarSize -= spacingBetweenElements;
                    LayoutCol(_itemList, currentBarSpace, currentBarSize, workingSize - (isChildForceExpandHeight ? 0 : spacingBetweenElements), newOffset, padding.top, axis);
                }
            }
        ...
        protected void LayoutRow(IList<RectTransform> contents, float rowWidth, float rowHeight, float maxWidth, float xOffset, float yOffset, int axis)
        {
            ...
            if (!isChildForceExpandWidth && IsCenterAlign)
            {
                xPos += (maxWidth - rowWidth) * 0.5f;
            }
            else if (!isChildForceExpandWidth && IsRightAlign)
            {
            ...
            for (var j = 0; j < _itemList.Count; j++)
            {
                ...
                var rowChildWidth = my_GetPreferredSize(rowChild, 0) + extraWidth;
                var rowChildHeight = my_GetPreferredSize(rowChild, 1);

                if (isChildForceExpandHeight)
                ...
                if (axis == 0)
                {
                    my_SetChildAlongAxis(rowChild, 0, xPos, rowChildWidth);
                }
                else
                {
                    my_SetChildAlongAxis(rowChild, 1, yPos, rowChildHeight);
                }
        ...
        protected void LayoutCol(IList<RectTransform> contents, float colWidth, float colHeight, float maxHeight, float xOffset, float yOffset, int axis)
        {
            ...
            if (!isChildForceExpandHeight && IsMiddleAlign)
            {
                yPos += (maxHeight - colHeight) * 0.5f;
            }
            else if (!isChildForceExpandHeight && IsLowerAlign)
            {
            ...
            for (var j = 0; j < _itemList.Count; j++)
            {
                ...
                var rowChildWidth = my_GetPreferredSize(rowChild, 0);
                var rowChildHeight = my_GetPreferredSize(rowChild, 1) + extraHeight;
        ...
        ////////// ALTERNATE BY RZ: //////////
        public float my_GetPreferredSize(RectTransform rect, int axis)
        {
            var widthOrHeight = axis == 0 ? childWidth : childHeight;
            if (widthOrHeight == sizeControl.Manual)
            {
        ...
        public float my_GetMinWidth(RectTransform rect)
        {
            return LayoutUtility.GetMinWidth(rect);
        }

        public float my_GetMinHeight(RectTransform rect)
        {
            return LayoutUtility.GetMinHeight(rect);
        }

        public void my_SetChildAlongAxis(RectTransform rect, int axis, float pos, float size)
        {
            var widthOrHeight = axis == 0 ? childWidth : childHeight;
            if (widthOrHeight == sizeControl.Manual)
            {
                SetChildAlongAxis(rect, axis, pos);
            }
            else
            {
