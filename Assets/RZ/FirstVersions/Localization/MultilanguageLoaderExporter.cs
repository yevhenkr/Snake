...
namespace RZ.Localizations
{
    ...
    public class MultilanguageLoaderExporter : MonoBehaviour
    {
        [Tooltip("The Localization target the translations will be loaded into. If this is empty, the first in the scene will automatically be used. If there are none in the scene, one will be created")]
        public Localization Target;
        ...
        public void LoadFromSource()
        {
            if (Source != null)
            {
                ...
                // Automatically get first Localization?
                if (target == null && Localization.AllLocalizations.Count > 0)
                {
                    target = Localization.AllLocalizations[0];
                }

                // Create a new Localization?
                if (target == null)
                {
                    target = new GameObject("Localization").AddComponent<Localization>();
                }
                ...
                for (var i = 0; i < lines.Length; i++)
                {
                    ...
                    if (equalsIndex != -1)
                    {
                        ...
                        if (colonIndex != -1)
                        {
                            ...
                            if (Localization.CurrentLanguage == languageName)
                            {
                                updateLanguage = true;
                            }
                        }
        ...
        public void ExportTextAsset()
        {
            ...
            if (target != null)
            {
                ...
                // var path = UnityEditor.EditorUtility.SaveFilePanel("Export Text Asset", "", "MultiLanguage", "txt");
